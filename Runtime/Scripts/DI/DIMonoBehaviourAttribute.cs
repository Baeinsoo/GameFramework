...
{
    /// <summary>
    /// 이 어트리뷰트가 붙은 MonoBehaviour는 자신이 속한 씬이 로드될 때
    /// SceneLifetimeScope의 스캔에 의해 VContainer가 주입한다 —
    /// [Inject]가 붙은 멤버(필드/프로퍼티/메서드)가 채워진다.
    ...
    /// - 컴포넌트 하나만 주입한다(자식 포함 X). GameObject 계층 전체를 주입하려면 [DIGameObject]를 사용한다.
    /// </summary>
    [AttributeUsage(AttributeTargets.Class)]
    public class DIMonoBehaviourAttribute : Attribute { }
}
